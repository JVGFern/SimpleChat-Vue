...
<template>
  <header
    id="header"
    class="fixed top-0 z-50 w-full transform bg-white px-6 py-5 transition-all duration-500 ease-in-out"
    :class="{
    ...
    <div class="mx-auto flex max-w-5xl flex-wrap items-center justify-between">
      <div class="sm:mr-8">
        <a class="flex items-center" href="/">
          <span class="self-center text-xl font-semibold text-teal-700">Lista Demo Vue</span>
        </a>
      </div>
      <nav
        id="menu"
        ...
        <RouterLink
          to="/"
          class="mr-4 mt-4 block text-base font-medium text-slate-700 hover:text-teal-600 md:mt-0 md:inline-block"
      ...
      <div
        id="menu-toggle"
        class="flex cursor-pointer items-center text-slate-700 hover:text-teal-600 sm:ml-6 md:hidden"
        @click="toggleMenu">
        <svg
